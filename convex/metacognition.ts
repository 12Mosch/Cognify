...
interface StudyStrategy {
  ...
  name: string;
  description: string;
  category: 'encoding' | 'retrieval' | 'elaboration' | 'organization' | 'metacognitive';
  ...
  handler: async (ctx, args) => {
    ...
    if (args.category) {
      ...
      if (args.sessionContext) {
        const { averageSuccess, cardsReviewed, sessionDuration } = args.sessionContext;
      ...
      if (prompts.length < 3) {
        const generalPrompts = [
          { category: 'understanding', prompt: "How well do you understand the material you just studied?", priority: 'medium' as const },
          { category: 'motivation', prompt: "How engaged did you feel during this session?", priority: 'low' as const },
          { category: 'goals', prompt: "How does today's learning contribute to your larger goals?", priority: 'low' as const },
        ];
...
export const getStrategyRecommendations = query({
  args: {
    context: v.optional(v.object({
      ...
      learningGoal: v.union(v.literal("memorization"), v.literal("understanding"), v.literal("application")),
      timeAvailable: v.number(), // minutes
      userLevel: v.union(v.literal("beginner"), v.literal("intermediate"), v.literal("advanced")),
    })),
  },
  returns: v.array(v.object({
    strategy: v.object({
  ...
  handler: async (ctx, args) => {
    ...
    for (const strategy of STUDY_STRATEGIES) {
      ...
      if (args.context) {
        const { difficulty, learningGoal, timeAvailable, userLevel } = args.context;
        ...
        // Goal matching
        if (learningGoal === 'memorization' && strategy.category === 'retrieval') {
          relevanceScore += 0.3;
          reasoning += ". Perfect for memorization goals";
        } else if (learningGoal === 'understanding' && strategy.category === 'elaboration') {
          relevanceScore += 0.3;
      ...
      if (learningPattern) {
        if (learningPattern.averageSuccessRate < 0.6 && strategy.category === 'retrieval') {
          relevanceScore += 0.2;
...
export const recordConfidenceCalibration = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const calibrationError = Math.abs(args.predictedConfidence - args.actualPerformance);
  ...
  handler: async (ctx, _args) => {
    ...
    if (calibrations.length === 0) {
      return {
        ...
        recommendations: ["Complete more reviews with confidence ratings to get insights"],
      };
    }

    const averageCalibrationError = calibrations.reduce((sum, c) => sum + c.calibrationError, 0) / calibrations.length;
    const overconfidenceBias = calibrations.reduce((sum, c) => sum + (c.predictedConfidence - c.actualPerformance), 0) / calibrations.length;

    // Calculate trend (compare first half vs second half)
    const midpoint = Math.floor(calibrations.length / 2);
    const recentError = calibrations.slice(0, midpoint).reduce((sum, c) => sum + c.calibrationError, 0) / midpoint;
    const olderError = calibrations.slice(midpoint).reduce((sum, c) => sum + c.calibrationError, 0) / (calibrations.length - midpoint);
    ...
    if (overconfidenceBias > 0.2) {
      recommendations.push("You tend to be overconfident. Try to be more conservative in your predictions.");
    } else if (overconfidenceBias < -0.2) {
      recommendations.push("You tend to underestimate your knowledge. Trust your understanding more.");
    }

    if (averageCalibrationError > 0.3) {
      recommendations.push("Focus on accurately assessing your knowledge before answering.");
    }

    if (calibrationTrend === "declining") {
      recommendations.push("Your calibration has been getting worse. Take more time to reflect on your confidence.");
    }
    ...
    return {
      ...
      overconfidenceBias,
      calibrationTrend,
      recommendations: recommendations.length > 0 ? recommendations : ["Your confidence calibration looks good! Keep it up."],
    };
  },
});
