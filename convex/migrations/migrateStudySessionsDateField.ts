	...
	handler: async (ctx) => {
		...
		for (const session of sessions) {
			...
			if (sessionAny.date && !sessionAny.sessionDate) {
				...
				console.warn(
					`Session ${session._id} has neither 'date' nor 'sessionDate' field`,
				);
			}
		}

		console.log(
			`Migration completed. Migrated ${migratedCount} study sessions, skipped ${skippedCount} already migrated sessions.`,
		);
		return {
