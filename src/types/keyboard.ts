...
export const KEYBOARD_SHORTCUTS = {
  ...
  UNIVERSAL: [
    ...
    { key: '?', description: 'Show keyboard shortcuts help', context: 'general' as const },
  ],
  ...
  SPACED_REPETITION: [
    { key: '1', description: 'Again (when answer is shown)', context: 'rating' as const },
    { key: '2', description: 'Hard (when answer is shown)', context: 'rating' as const },
    { key: '3', description: 'Good (when answer is shown)', context: 'rating' as const },
    { key: '4', description: 'Easy (when answer is shown)', context: 'rating' as const },
  ],
} as const;
 ...
 */
export function getKeyboardShortcuts(mode: 'basic' | 'spaced-repetition'): KeyboardShortcutGroup[] {
 ...
 */
export function isShortcutKey(event: KeyboardEvent, shortcutKey: string): boolean {
  // Handle special keys
  switch (shortcutKey) {
    case 'Space':
