 ...
 */
export default function AdaptiveStudyMode({ deckId, onExit }: AdaptiveStudyModeProps) {
  ...
  const reviewCardAdaptive = useMutation(api.adaptiveLearning.reviewCardAdaptive);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800 flex flex-col">
          ...
          <div className="flex items-center gap-4 text-sm">
            <div className="flex items-center gap-2">
              ...
              <span className="text-blue-700 dark:text-blue-300">
                {t('study.successRate', 'Success Rate')}: {Math.round(learningPattern.averageSuccessRate * 100)}%
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-blue-600 dark:text-blue-400">⚡</span>
              <span className="text-blue-700 dark:text-blue-300">
                {t('study.learningVelocity', 'Learning Velocity')}: {learningPattern.learningVelocity.toFixed(1)} {t('study.cardsPerDay', 'cards/day')}
              </span>
            </div>
          </div>
        </div>
      ...
      {personalizedMessage && (
        <div className="p-4 bg-green-50 dark:bg-green-900/20 border-b border-green-200 dark:border-green-800">
          <div className="flex items-center gap-2">
            ...
            <span className="text-green-700 dark:text-green-300">{personalizedMessage}</span>
          </div>
        </div>
      )}
      ...
      <div className="p-4 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border-t border-slate-200 dark:border-slate-700">
        <div className="text-center text-sm text-slate-600 dark:text-slate-400">
          {t('study.keyboardShortcuts', 'Keyboard shortcuts')}: Space/Enter {t('study.toFlip', 'to flip')}, 1-4 {t('study.toRate', 'to rate')}
        </div>
      </div>
