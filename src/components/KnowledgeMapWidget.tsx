				...
				>
					...
					<div className="flex items-center gap-4 text-sm">
						<div className="flex items-center gap-2">
							<span className="text-slate-600 dark:text-slate-400 hover:scale-110 transition-transform duration-200">
								📊
							</span>
	...
	return (
		<div className="space-y-4">
			...
			{paths.map((path, index: number) => (
				<div
					key={index}
					className="p-4 bg-slate-50 dark:bg-slate-700 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-600 hover:scale-[1.02] transition-all duration-200 cursor-pointer hover:shadow-md"
				>
					<div className="flex items-start justify-between mb-3">
						<div>
							<h4 className="font-semibold text-slate-900 dark:text-slate-100 mb-1 hover:text-slate-700 dark:hover:text-slate-200 transition-colors">
				...
				<div className="space-y-2">
					{["similar", "related", "prerequisite"].map((type) => {
						...
						return (
							<div
								key={type}
								className="flex items-center justify-between text-sm hover:bg-slate-100 dark:hover:bg-slate-700 p-2 rounded transition-colors duration-200 cursor-pointer"
							>
			...
			<div className="animate-pulse space-y-4">
				...
				<div className="flex gap-2">
					{[...Array(3)].map((_, i) => (
						<div
							key={i}
							className="h-10 bg-slate-200 dark:bg-slate-700 rounded w-24"
						/>
					))}
				</div>
