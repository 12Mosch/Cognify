 ...
 */
export const DeckPerformanceTableSkeleton = memo(function DeckPerformanceTableSkeleton() {
  return (
    <div className="bg-slate-50 dark:bg-slate-800 p-6 rounded-lg border-2 border-slate-200 dark:border-slate-700">
      {/* Table Header */}
      ...
      <div className="grid grid-cols-5 gap-4 mb-4 pb-2 border-b border-slate-200 dark:border-slate-700">
        {Array.from({ length: 5 }).map((_, index) => (
          <div key={index} className="h-4 bg-slate-200 dark:bg-slate-700 rounded animate-skeleton-pulse"></div>
        ))}
      </div>
      
      {/* Table Rows */}
      {Array.from({ length: 4 }).map((_, rowIndex) => (
        <div key={rowIndex} className="grid grid-cols-5 gap-4 py-3 border-b border-slate-100 dark:border-slate-800">
          {Array.from({ length: 5 }).map((_, colIndex) => (
            <div key={colIndex} className="h-4 bg-slate-200 dark:bg-slate-700 rounded animate-skeleton-pulse"></div>
          ))}
        </div>
      ))}
    </div>
...
export const UpcomingReviewsSkeleton = memo(function UpcomingReviewsSkeleton() {
  return (
    <div className="bg-slate-50 dark:bg-slate-800 p-6 rounded-lg border-2 border-slate-200 dark:border-slate-700">
      ...
      {/* Review Items */}
      {Array.from({ length: 5 }).map((_, index) => (
        <div key={index} className="flex items-center justify-between py-3 border-b border-slate-100 dark:border-slate-800 last:border-b-0">
