...
describe("StudyProgressBar", () => {
	...
	it("calculates percentage correctly for various positions", () => {
		const { rerender } = render(
			<StudyProgressBar currentPosition={1} totalCards={3} />,
		);

		// 1 of 3 = 33%
		expect(screen.getByText("33%")).toBeInTheDocument();

		rerender(<StudyProgressBar currentPosition={2} totalCards={3} />);

		// 2 of 3 = 67%
		expect(screen.getByText("67%")).toBeInTheDocument();

		rerender(<StudyProgressBar currentPosition={3} totalCards={3} />);
	...
	it("has proper accessibility attributes", () => {
		render(<StudyProgressBar currentPosition={7} totalCards={15} />);
		...
		expect(
			screen.getByText(/Study session in progress: 7 of 15 cards completed/),
		).toBeInTheDocument();
	});
	...
	it("shows correct colors for in-progress vs completed states", () => {
		...
		expect(completedProgressBar).toHaveClass(
			"bg-green-500",
			"dark:bg-green-400",
		);
	});
