...
export function generateHeatmapGrid(
  ...
  const studyDataMap = new Map(
    studyData.map(item => [item.date, item])
  );
...
export function calculateHeatmapStats(data: HeatmapData) {
  ...
  const totalSessions = activeDays.reduce((sum, day) => sum + day.sessionCount, 0);
  const totalTime = activeDays.reduce((sum, day) => sum + (day.totalDuration || 0), 0);

  const maxCardsInDay = Math.max(...activeDays.map(day => day.cardsStudied), 0);
  const averageCardsPerActiveDay = activeDays.length > 0 
    ? Math.round(totalCards / activeDays.length) 
    : 0;
