...
describe('Retention Rate Calculation', () => {
  ...
  const calculateRetentionRate = (reviews: any[], daysPeriod: number = 30): number | undefined => {
    const cutoffDate = Date.now() - (daysPeriod * 24 * 60 * 60 * 1000);
    
    // Filter reviews within time period
    const recentReviews = reviews.filter(review => review.reviewDate >= cutoffDate);
    ...
    const successfulReviews = recentReviews.filter(review => review.wasSuccessful).length;
